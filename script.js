...
function buildDefaultQuestionMap() {
  ...
  for (const [category, questions] of Object.entries(DEFAULT_QUESTIONS)) {
    map[category] = questions
      // 1. Target the .title property of the question object
      .filter((q) => !deletedQuestions[normalizeKey(category, q.title)])
      // 2. Pass all the required properties into makeQuestion
      .map((q) => makeQuestion(q.title, q.difficulty, q.link));
  }
